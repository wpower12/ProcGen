...
module.exports = function (grunt) {
  ...
  grunt.registerTask('buildBootstrapper', 'builds the bootstrapper file correctly', function() {
    ...
    var plugFiles = grunt.file.expand('game/plugins/*.js');
    var plugins = [];
    plugFiles.forEach(function(file) {
      var plugin = file.match(statePattern)[1];
      if (!!plugin) {
        plugins.push({shortName: plugin, pluginName: _.capitalize(plugin)});
      }
    });
    
    plugFiles = grunt.file.expand('game/generation/*.js');
    plugFiles.forEach(function(file) {
      var plugin = file.match(statePattern)[1];
      if (!!plugin) {
        plugins.push({shortName: plugin, pluginName: _.capitalize(plugin)});
